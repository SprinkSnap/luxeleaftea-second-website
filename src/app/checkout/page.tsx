...
export const metadata = createMetadata({
  title: "Checkout",
  description: "Secure Lux Leaf Tea checkout with guest support.",
  path: "/checkout",
  noIndex: true,
});
    ...
    <div className="container-wide grid gap-10 px-4 py-10 lg:grid-cols-[1.1fr_0.9fr] lg:py-14">
      <div>
        ...
        <p className="mt-2 text-brand-muted">
          Guest checkout supported. Account creation is optional.
        </p>
        <div className="mt-8">
          <CheckoutForm />
        </div>
      </div>
      <aside className="h-fit rounded-[var(--radius-lg)] border border-[var(--brand-line)] bg-white/80 p-5 lg:sticky lg:top-36">
        ...
        <ul className="mt-4 space-y-3">
          {cart.items.map((item) => (
            <li key={item.id} className="flex justify-between gap-3 text-sm">
              ...
              <span>{formatMoney(item.variant.retailPrice * item.quantity)}</span>
            </li>
          ))}
        </ul>
        ...
        <p className="mt-4 text-xs text-brand-muted">
          Secure payment via Stripe when configured.{" "}
